...
try {
  ...
  $preferredWorkerBase = "https://dyrakarmy.online"
  $fallbackWorkerBase = "https://sounddrop.biramentv.workers.dev"
  ...
  $workerBase = $preferredWorkerBase
  try {
    Test-EndToEnd -WorkerBase $workerBase
  } catch {
    Write-Info "Preferred domain health failed, falling back to workers.dev for smoke..."
    $workerBase = $fallbackWorkerBase
    Test-EndToEnd -WorkerBase $workerBase
  }
