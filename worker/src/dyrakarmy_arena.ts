...
const QUESTION_BANK: ArenaQuestion[] = [
  ...
  { id: 'routing-archive', category: 'routing', prompt: 'Къде се пази Telegram file_id за повторно изпращане?', options: ['Само в браузърния history', 'В общата D1/Telegram archive логика', 'В DNS', 'В favicon'], correct: 1 },
...
export function calculateArenaScore(
  ...
  answers: ArenaAnswerInput[],
): ArenaScore {
  const answerMap = new Map(answers.map((answer) => [String(answer.question_id || ''), answer]));
  let score = 0;
  let correct = 0;
  let combo = 0;
  let bestCombo = 0;
  ...
  for (const question of questions) {
    ...
    const responseMs = clamp(Math.round(Number(answer?.response_ms) || 15000), 150, 15000);
    responseTimes.push(responseMs);
    if (option === question.correct) {
      correct += 1;
      combo += 1;
      bestCombo = Math.max(bestCombo, combo);
      const speedBonus = responseMs < 250 ? 0 : Math.max(0, Math.round(650 - responseMs / 12));
  ...
  if (correct === questions.length) score += 1500;
  score += accuracy * 12;
  const avgResponseMs = responseTimes.length
    ? Math.round(responseTimes.reduce((sum, value) => sum + value, 0) / responseTimes.length)
    : 15000;
  const xp = clamp(Math.round(score / 25), 35, 650);
  return {
    ...
    team_points: Math.max(0, Math.round(score)),
  };
}
...
export async function handleDyrakArmyArenaApi(request: Request, env: ExtendedEnv): Promise<Response | null> {
  ...
  if (url.pathname === '/api/games/dyrakarmy-arena/leaderboard' && request.method === 'GET') {
    ...
    return json(request, {
      ...
      season_key: arenaSeasonKey(),
      entries: scope === 'teams'
        ? await teamLeaderboard(env, period, limit)
        : await playerLeaderboard(env, period, limit),
    });
  }

  if (request.method !== 'POST') {
    return json(request, { error: { code: 'METHOD_NOT_ALLOWED', message: 'Method not allowed' } }, 405);
  }
  ...
  if (url.pathname === '/api/games/dyrakarmy-arena/session') {
    ...
    if (user) {
      const attempts = await attemptsToday(user.id, env);
  ...
  if (url.pathname === '/api/games/dyrakarmy-arena/team') {
    ...
    if (action === 'create') {
      ...
      const existing = await membership(user.id, env);
      if (existing) return json(request, { error: { code: 'ALREADY_IN_TEAM', message: 'Leave the current team first' } }, 409);
    ...
    if (action === 'join') {
      ...
      await env.DB.prepare("INSERT INTO arena_team_members (team_id, telegram_user_id, role) VALUES (?, ?, 'member')")
    ...
    if (action === 'leave') {
      ...
      const count = await env.DB.prepare('SELECT COUNT(*) AS count FROM arena_team_members WHERE team_id = ?')
        .bind(current.team_id).first<{ count: number }>();
  ...
  if (url.pathname === '/api/games/dyrakarmy-arena/score') {
    ...
    if (session.user_id !== user.id || session.practice) return json(request, { error: { code: 'SESSION_MISMATCH', message: 'Session mismatch' } }, 403);
    if (session.day_key !== arenaDayKey()) return json(request, { error: { code: 'CHALLENGE_EXPIRED', message: 'Daily challenge changed' } }, 409);
    const attempts = await attemptsToday(user.id, env);
    if (attempts >= DAILY_ATTEMPTS) return json(request, { error: { code: 'DAILY_LIMIT', message: 'Daily ranked attempts are complete' } }, 409);
    ...
    const seasonKey = arenaSeasonKey();
    ...
    await env.DB.batch([
      ...
      `).bind(
        runId, user.id, team?.id || null, session.day_key, weekKey, seasonKey,
        result.score, result.correct, result.total, result.accuracy, result.avg_response_ms,
        result.best_combo, result.xp,
      ),
      env.DB.prepare(`
    ...
    return json(request, {
      ...
      result,
      team,
      ...(await buildArenaProfile(user.id, env)),
    });
  }
...
export async function getDyrakArmyArenaBotSummary(
  ...
  if (language === 'en') {
    return [
      ...
      `🔑 Team code: ${team?.code || '—'}`,
      `🏅 Shared rank: ${rank.name}`,
      `⚡ Shared XP: ${Number(profile?.total_xp || 0)}`,
      `🏆 Weekly player position: ${weekly ? `#${weekly}` : '—'}`,
      `🎯 Ranked attempts today: ${attempts}/${DAILY_ATTEMPTS}`,
    ].join('\n');
  }
  return [
    '⚔️ DyrakArmy Arena', '',
    `🛡 Отбор: ${team?.name || 'Нямаш отбор'}`,
    `🔑 Код на отбора: ${team?.code || '—'}`,
    `🏅 Общ ранг: ${rank.name}`,
    `⚡ Общ XP: ${Number(profile?.total_xp || 0)}`,
    `🏆 Седмична лична позиция: ${weekly ? `#${weekly}` : '—'}`,
    `🎯 Ranked опити днес: ${attempts}/${DAILY_ATTEMPTS}`,
  ].join('\n');
}
...
async function teamForUser(userId: number, env: ExtendedEnv): Promise<TeamRow | null> {
  const row = await env.DB.prepare(`
    ...
    FROM arena_team_members m
    JOIN arena_teams t ON t.id = m.team_id
...
async function buildArenaProfile(userId: number, env: ExtendedEnv) {
  ...
  return {
    profile: profile ? { ...profile, rank: latencyStrikeRank(totalXp) } : null,
...
async function playerLeaderboard(env: ExtendedEnv, period: 'week' | 'season', limit: number) {
  ...
  const result = await env.DB.prepare(`
    ...
    FROM arena_runs r
    JOIN game_profiles p ON p.telegram_user_id = r.telegram_user_id
    WHERE ${column} = ?
    GROUP BY p.telegram_user_id
    ORDER BY points DESC, best_score DESC, games ASC
    LIMIT ?
...
async function teamLeaderboard(env: ExtendedEnv, period: 'week' | 'season', limit: number) {
  ...
  const value = period === 'week' ? latencyStrikeWeekKey() : arenaSeasonKey();
  const result = await env.DB.prepare(`
    SELECT t.id, t.code, t.name, COUNT(DISTINCT m.telegram_user_id) AS members,
      COALESCE(SUM(r.score), 0) AS points, COUNT(r.id) AS games
    FROM arena_teams t
    LEFT JOIN arena_team_members m ON m.team_id = t.id
    LEFT JOIN arena_runs r ON r.team_id = t.id AND ${column} = ?
    GROUP BY t.id
    ORDER BY points DESC, members DESC, t.created_at ASC
    LIMIT ?
  `).bind(value, limit).all<Record<string, unknown>>();
  return (result.results || []).map((row, index) => ({ position: index + 1, ...row }));
}
...
async function uniqueTeamCode(env: ExtendedEnv): Promise<string> {
  for (let attempt = 0; attempt < 10; attempt += 1) {
    ...
    const exists = await env.DB.prepare('SELECT 1 AS ok FROM arena_teams WHERE code = ? LIMIT 1').bind(code).first();
