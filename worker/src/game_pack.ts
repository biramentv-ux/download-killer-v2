...
export interface GamePackQuestion { id: string; prompt: string; options: string[]; correct: number; explanation: string; clue?: string }
export interface GamePackDefinition {
  id: GamePackId; title: string; icon: string; command: string; description: string;
  mechanic: string; reward_id: string; reward_title: string; questions: GamePackQuestion[];
}
interface GameAnswerInput { question_id?: string; option_index?: number; response_ms?: number }
interface GamePackSession {
  user_id: number; game_id: GamePackId; practice: boolean; day_key: string;
  question_ids: string[]; bot_score: number; issued_at: number;
}
export interface GamePackScore {
  score: number; xp: number; correct: number; total: number; accuracy: number;
  avg_response_ms: number; best_combo: number; bot_score: number | null; won_duel: boolean | null;
}
