...
ENV_URL_LEGACY = 'SOUNDDROP_URL'
ENV_SYNC_KEY_LEGACY = 'SOUNDDROP_SYNC_KEY'
ENV_LANG = 'DYRAKARMY_LANG'
ENV_LANG_LEGACY = 'SOUNDDROP_LANG'
SYNC_KEY_FILENAME = 'dyrakarmy_sync_key.txt'
HEALTH_PATH = '/api/health'
RUNTIME_CONFIG_PATH = '/api/runtime-config'
REQUEST_TIMEOUT_SECONDS = 4
RUNTIME_CONFIG_TTL_SECONDS = 5 * 60
RUNTIME_CONFIG_FILE = 'dyrakarmy_runtime_config.json'
LOG_FILE = Path(tempfile.gettempdir()) / 'dyrakarmy_desktop.log'
...
def normalize_preferred_directory(raw: str) -> Path | None:
...
def local_tool_dir() -> Path:
  ...
  path = base / 'DyrakArmyDesktop' / 'tools'
...
def extract_artist_from_description(description: str) -> str:
  if not description:
    return ''
  # Spotify/Apple pages often use bullet separators: "Artist В· Album В· Song В· 2026"
  parts = [segment.strip() for segment in description.split('В·') if segment.strip()]
...
def load_sidecar_urls() -> list[str]:
    ...
    txt_path = next((candidate for candidate in [
        base / 'dyrakarmy_desktop.url',
        base / 'sounddrop_desktop.url',
    ] if candidate.exists()), base / 'dyrakarmy_desktop.url')
    if txt_path.exists():
        try:
    ...
    json_path = next((candidate for candidate in [
        base / 'dyrakarmy_desktop.json',
        base / 'sounddrop_desktop.json',
    ] if candidate.exists()), base / 'dyrakarmy_desktop.json')
    if json_path.exists():
        try:
...
def candidate_urls() -> list[str]:
    env_raw = os.getenv(ENV_URL, '') or os.getenv(ENV_URL_LEGACY, '')
...
def resolve_sync_key() -> str:
    from_env = (os.getenv(ENV_SYNC_KEY, '') or os.getenv(ENV_SYNC_KEY_LEGACY, '')).strip()
    ...
    generated = f'da{uuid.uuid4().hex[:20]}'
...
def detect_preferred_language() -> str:
    env_lang = (os.getenv(ENV_LANG, '') or os.getenv(ENV_LANG_LEGACY, '')).strip().lower()
    if env_lang in {'bg', 'en', 'es', 'ru', 'de'}:
        return env_lang
...
def attach_launcher_params(base_url: str, language: str) -> str:
    ...
    if language in {'bg', 'en', 'es', 'ru', 'de'} and 'lang' not in query:
        query['lang'] = [language]
    encoded = urllib.parse.urlencode(query, doseq=True)
    return urllib.parse.urlunsplit((parsed.scheme, parsed.netloc, parsed.path, encoded, parsed.fragment))
...
def load_cached_runtime_config() -> dict[str, Any] | None:
    ...
    try:
        ...
        log(f'Failed to read runtime config cache: {error}')
        return None
    if not isinstance(payload, dict):
        return None
    ...
    if saved_at <= 0:
        return None
    age_seconds = max(0, int(datetime.now(timezone.utc).timestamp()) - saved_at)
    if age_seconds > RUNTIME_CONFIG_TTL_SECONDS:
        return None
    config = payload.get('config')
    return config if isinstance(config, dict) else None


def save_runtime_config_cache(config: dict[str, Any]) -> None:
    path = runtime_config_path()
    try:
        path.write_text(
            json.dumps(
        ...
        log(f'Failed to write runtime config cache: {error}')


def derive_base_from_runtime_config(config: dict[str, Any], fallback_base: str) -> str:
    public_base = normalize_url(str(config.get('public_base') or ''))
    if public_base:
        return public_base

    api_base = normalize_url(str(config.get('api_base') or ''))
    if api_base:
        try:
...
def parse_semver(raw: str) -> tuple[int, int, int]:
    parts: list[int] = []
    for chunk in str(raw or '0.0.0').split('.'):
        try:
...
def resolve_update_url(candidate: str, base_url: str) -> str:
    value = str(candidate or '').strip()
    if not value:
        return ''
    if value.startswith('http://') or value.startswith('https://'):
        return value
    return urllib.parse.urljoin(f'{strip_query(base_url)}/', value)
...
def resolve_forced_update_url(runtime_config: dict[str, Any], base_url: str) -> str:
    client_min = runtime_config.get('client_min_versions')
    if not isinstance(client_min, dict):
        return ''

    min_key = 'desktop_macos' if APP_PLATFORM == 'macos' else 'desktop_windows'
    min_version = str(client_min.get(min_key) or '0.0.0')
    if compare_semver(APP_VERSION, min_version) >= 0:
        return ''

    updates = runtime_config.get('updates')
    if isinstance(updates, dict):
        channel_key = 'desktop_macos' if APP_PLATFORM == 'macos' else 'desktop_windows'
        channel = updates.get(channel_key)
        if isinstance(channel, dict):
            update_url = resolve_update_url(str(channel.get('download_url') or ''), base_url)
            if update_url:
                return update_url

    downloads = runtime_config.get('downloads')
    if isinstance(downloads, dict):
        fallback_key = 'macos_portable' if APP_PLATFORM == 'macos' else 'windows_exe'
        update_url = resolve_update_url(str(downloads.get(fallback_key) or ''), base_url)
        if update_url:
            return update_url
...
def fetch_runtime_config(base_url: str) -> dict[str, Any] | None:
    config_url = f'{base_url}{RUNTIME_CONFIG_PATH}'
    request = urllib.request.Request(
        config_url,
        headers={
            'User-Agent': APP_USER_AGENT,
    ...
    try:
        ...
        log(f'Runtime config fetch failed for {base_url}: {error}')
        return None
...
def resolve_target_url() -> str:
    ...
    if not selected and urls:
        selected = urls[0]
        log(f'No healthy endpoint found, using first candidate: {selected}')

    if not selected:
        selected = DEFAULT_PRIMARY_URL
        log(f'No candidates detected, fallback to default: {selected}')

    live_config = fetch_runtime_config(selected)
    if live_config:
        save_runtime_config_cache(live_config)
        runtime_base = derive_base_from_runtime_config(live_config, selected)
        if runtime_base:
            log(f'Using runtime-config base: {runtime_base}')
            return runtime_base

    cached = load_cached_runtime_config()
    if cached:
        runtime_base = derive_base_from_runtime_config(cached, selected)
        if runtime_base:
            log(f'Using cached runtime-config base: {runtime_base}')
            return runtime_base
...
class DesktopBridge:
    ...
    def ping(self) -> dict[str, Any]:
        return {'ok': True, 'bridge': 'dyrakarmy-desktop'}
    ...
    def download_track(
        ...
        if is_playlist_url(url):
            return self.download_playlist(url, audio_format, audio_quality, source, preferred_directory)
        ...
        target_path = self._pick_save_target(suggested_name, preferred_directory)
        if target_path is None:
            return {'ok': False, 'cancelled': True}
    ...
    def download_playlist(
        ...
        folder = self._pick_save_folder(playlist.get('title') or 'Playlist', preferred_directory)
        if folder is None:
            return {'ok': False, 'cancelled': True}
    ...
    def _pick_save_folder(self, playlist_title: str, preferred_directory: str = '') -> Path | None:
        if self.window is None:
            return None

        preferred_base = normalize_preferred_directory(preferred_directory) or (Path.home() / 'Downloads')
        default_dir = preferred_base / sanitize_file_component(playlist_title, 'Playlist')
        try:
    ...
    def _pick_save_target(self, suggested_filename: str, preferred_directory: str = '') -> Path | None:
        if self.window is None:
            return None
        ...
        base_dir = normalize_preferred_directory(preferred_directory) or (Path.home() / 'Downloads')
    ...
    def _run_download(self, url: str, audio_format: str, audio_quality: str) -> tuple[Path, dict[str, Any]]:
        ...
        work_dir = Path(tempfile.mkdtemp(prefix='dyrakarmy-desktop-'))
...
<body>
  <div class="card">
    ...
    <div class="title">Dyrak<span>Army</span></div>
    <div class="meta">Launching synced desktop shell...</div>
...
def main() -> None:
    ...
    try:
        runtime_config = fetch_runtime_config(strip_query(target_url))
        if runtime_config:
            update_url = resolve_forced_update_url(runtime_config, target_url)
            if update_url:
                log(f'Forced desktop update required -> opening {update_url}')
                webbrowser.open(update_url)
    except Exception as error:
        log(f'Forced update pre-check failed: {error}')
    try:
        import webview  # type: ignore

        bridge = DesktopBridge()
        window = webview.create_window(
            title='DyrakArmy Desktop',
