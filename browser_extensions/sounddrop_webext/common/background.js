(() => {
  ...
  async function fetchRuntimeConfig(base) {
    ...
    if (!payload || typeof payload !== 'object') {
      throw new Error('runtime-config payload invalid');
    }
    return payload;
  }
  ...
  async function resolveApiBaseWithRuntime(base) {
    ...
    if (cached && typeof cached === 'object') {
      ...
      if (savedAt > 0 && now - savedAt < RUNTIME_CONFIG_CACHE_TTL_MS) {
        return cachedBase;
      }
    }
    ...
    if (cached && typeof cached === 'object' && cached.baseUrl) {
      return normalizeApiBase(cached.baseUrl);
    }
  ...
  function getExtensionUpdateUrlFromRuntime(payload, fallbackBase) {
    ...
    if (typeof extension.chrome_zip_url === 'string' && extension.chrome_zip_url.trim()) {
      return extension.chrome_zip_url.trim();
    }
    return fallback;
  }
  ...
  async function getConfig() {
    ...
    try {
      const runtimeResolvedBase = await resolveApiBaseWithRuntime(config.apiBase);
      config.apiBase = normalizeApiBase(runtimeResolvedBase);
    } catch {
    ...
    if (stored.sd_api_base && normalizeApiBase(stored.sd_api_base) !== config.apiBase) {
      updates.sd_api_base = config.apiBase;
    }
    ...
    if (!stored.sd_download_directory) updates.sd_download_directory = config.downloadDirectory;
    if (!stored.sd_telegram_link_mode) updates.sd_telegram_link_mode = config.telegramLinkMode;
    if (!stored[PREFS_REVISION_KEY]) updates[PREFS_REVISION_KEY] = config.prefsRevision;

    try {
      const prefs = await fetch(`${config.apiBase}/api/preferences?key=${encodeURIComponent(syncKey)}`).then((r) => r.ok ? r.json() : null).catch(() => null);
      if (prefs && typeof prefs === 'object') {
        ...
        if (typeof prefs.download_directory === 'string') {
          config.downloadDirectory = normalizeDownloadDirectory(prefs.download_directory);
        }
        if (prefs.telegram_link_mode) {
          config.telegramLinkMode = normalizeTelegramLinkMode(prefs.telegram_link_mode);
        }
        if (Object.prototype.hasOwnProperty.call(prefs, 'revision')) {
          config.prefsRevision = normalizeRevision(prefs.revision);
        }
      }
    } catch {
    ...
    updates.sd_lang = config.lang;
    updates.sd_source = config.source;
    updates.sd_format = config.format;
    updates.sd_quality = config.quality;
    updates.sd_download_directory = config.downloadDirectory;
    updates.sd_telegram_link_mode = config.telegramLinkMode;
    updates[PREFS_REVISION_KEY] = config.prefsRevision;
    ...
    try {
      ...
      if (response.ok && payload && typeof payload === 'object') {
        if (Object.prototype.hasOwnProperty.call(payload, 'revision')) {
          merged.prefsRevision = normalizeRevision(payload.revision);
        }
        ...
        if (typeof payload.download_directory === 'string') {
          merged.downloadDirectory = normalizeDownloadDirectory(payload.download_directory);
        }
        if (payload.telegram_link_mode) {
          merged.telegramLinkMode = normalizeTelegramLinkMode(payload.telegram_link_mode);
        }
        await storageSet({
          sd_lang: merged.lang,
          sd_source: merged.source,
  ...
  async function queueAndResolve(userInput) {
    ...
    await notify('DyrakArmy', `Задачата #${shortId} е добавена.`);
    ...
    await notify('DyrakArmy', `Готово: ${title}`);
  ...
  async function setupContextMenus() {
    try {
      ...
      await contextMenusCreate({
        ...
        title: 'Свали линка със DyrakArmy',
        contexts: ['link'],
      });
      await contextMenusCreate({
        id: MENU_IDS.downloadMedia,
        title: 'Свали медия със DyrakArmy',
        contexts: ['audio', 'video'],
      });
      await contextMenusCreate({
        id: MENU_IDS.downloadPage,
        title: 'Свали текущата страница със DyrakArmy',
        contexts: ['page'],
      });
      await contextMenusCreate({
        id: MENU_IDS.openWeb,
        title: 'Отвори DyrakArmy Web App',
        contexts: ['action'],
      });
    } catch (error) {
